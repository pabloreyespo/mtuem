#' Likelihood function for the parameter estimation of a Microeconomic Time-Use-Expenditure Model (Theta-Phi parameterization).
#'
...
#'
#' @details
#' This function uses the Theta-Phi parameterization of the MTUEM. The work equation is specified
#' through \code{work_elasticities} with mandatory names \code{Theta}, \code{Phi}, and \code{thw}.
#' The optimal work time is computed as:
#'
#' \deqn{T_w^* = 0.5 * (tau - T_c) * (Phi + thw + (Theta + thw) * e_c + sqrt(D)) / (Theta + Phi + thw)}
#'
#' where \eqn{e_c = E_c / (w * (tau - T_c))} and \eqn{D} is the discriminant of the quadratic.
#'
...
#'                        \item \strong{\code{work_elasticities}} Named list. Value of the elasticity parameters associated with the work equation. Mandatory names are \code{Theta}, \code{Phi}, and \code{thw} (theta work).
...
#'
#' apollo_initialise()
#' apollo_control <- list(modelName = "maed-1eq", indivID = "PeID")
#'
