...
After estimation, use `mtuem_get_corr(model, apollo_probabilities, apollo_inputs, vars)` where `vars` is a character vector of the time-use-expenditure column names. Alternatively, call the likelihood with `functionality = "get_covar"`.
...
Estimate both standard deviations and correlations. For 3 equations, the upper triangle of the correlation matrix has 3 elements: rho[1,2], rho[1,3], rho[2,3]. These must be ordered horizontally.
...
This example shows the latent class framework with a single class and one equation. This serves as a stepping stone to multi-class models.

```{r example-lc-1class, eval = FALSE}
library(mtuem)
library(apollo)
...
After estimation, compute the Value of Leisure Time (VoL) and Value of Time Assigned to Work (VTAW):

```{r post-vot, eval = FALSE}
pred <- apollo_prediction(model, apollo_probabilities, apollo_inputs)
